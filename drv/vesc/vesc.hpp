...
class VescCommBase {
 public:
  VescCommBase(Usart* serial) : serial_(serial) {}
...
class VescComm : public VescCommBase {
 public:
  VescComm(Usart* serial) : VescCommBase(serial) {
    memset(&mc_values_, 0, sizeof(mc_values));
  }

  VescComm(uint8_t id) : VescCommBase(nullptr) {
 ...
 private:
  ...
  int actual_header_size() { return rx_data_[0]; }

  // buffer_pos_ always points  to non-written yet memory. bufferpos = 1 means
  // have only one byte with index 0
...
class VescServer : public VescCommBase {
 public:
  using VescCommBase::VescCommBase;

  int update();

  void sendStatus(const mc_values& values, uint8_t packet_id = 4);
