...
static float buffer_get_float16(const uint8_t *buffer, float scale,
                                int32_t *index) {
  ...
  if (buffer_pos_ > kHeaderSize) {
    ...
    if (buffer_pos_ >=
      ...
      if (msg_id == (uint8_t)COMM_PACKET_ID::COMM_GET_VALUES) {
        ...
        mc_values_.temp_motor_filtered =
            buffer_get_float16(rx_data_, 1e1, &idx);
...
uint8_t comm_can_transmit_eid_replace(uint32_t id, const uint8_t *data,
                                      uint8_t len, bool replace) {
  CanTxMsg TxMessage;
  TxMessage.ExtId = id;
...
void VescComm::processCanMessage(uint32_t ext_id, const uint8_t *data8) {
  ...
  switch (cmd) {
    case CAN_PACKET_STATUS:
      mc_values_.rpm = (float)buffer_get_int32(data8, &ind);
      mc_values_.avg_motor_current =
          (float)buffer_get_int16(data8, &ind) / 10.0;
      mc_values_.duty_now = (float)buffer_get_int16(data8, &ind) / 1000.0;
      break;

    case CAN_PACKET_STATUS_2:
      mc_values_.amp_hours = (float)buffer_get_int32(data8, &ind) / 1e4;
      mc_values_.amp_hours_charged = (float)buffer_get_int32(data8, &ind) / 1e4;
      break;
    ...
    case CAN_PACKET_STATUS_4:
      ...
      mc_values_.temp_motor_filtered =
          (float)buffer_get_int16(data8, &ind) / 10.0;
      mc_values_.avg_input_current =
          (float)buffer_get_int16(data8, &ind) / 10.0;
      // stat_tmp_4->pid_pos_now = (float)buffer_get_int16(data8, &ind) / 50.0;
      break;

    case CAN_PACKET_STATUS_5:
      mc_values_.tachometer =
          buffer_get_int32(data8, &ind);  // is this tacho_abs?
